from  src.fr.hymaia.exo4.no_udf import calculate_total_price_per_category_per_day_last_30_days , calculate_total_price_per_category_per_day
import unittest
...
class TestNoUdf(unittest.TestCase):
    ...
    def test_calculate_total_price_per_category_per_day_last_30_days(self):
        ...
        actual_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"],row["total_price_per_category_per_day_last_30_days"]) for row in actual.collect()]
        ...
        expected_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"], row["total_price_per_category_per_day_last_30_days"]) for row in df_expected.collect()]
    ...
    def test_calculate_total_price_per_category_per_day(self):
        ...
        actual_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"],row["total_price_per_category_per_day"]) for row in actual.collect()]
        ...
        expected_list = [(row["id"], row["date"], row["category"], row["price"], row["category_name"], row["total_price_per_category_per_day"]) for row in df_expected.collect()]
